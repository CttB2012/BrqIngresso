// <auto-generated />
using System;
using BRQ_Ingresso.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace BRQ_Ingresso.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20230503143747_'PrimeiraMigracao'")]
    partial class PrimeiraMigracao
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("BRQ_Ingresso.Models.EnderecoUsuario", b =>
                {
                    b.Property<string>("IdEndereco")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Bairro")
                        .HasColumnType("longtext");

                    b.Property<string>("Cep")
                        .HasColumnType("longtext");

                    b.Property<string>("Cidade")
                        .HasColumnType("longtext");

                    b.Property<string>("Estado")
                        .HasColumnType("longtext");

                    b.Property<string>("IdUsuario")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Logradouro")
                        .HasColumnType("longtext");

                    b.Property<string>("Numero")
                        .HasColumnType("longtext");

                    b.Property<string>("Pais")
                        .HasColumnType("longtext");

                    b.HasKey("IdEndereco");

                    b.ToTable("EnderecoUsuarios");
                });

            modelBuilder.Entity("BRQ_Ingresso.Models.Usuario", b =>
                {
                    b.Property<string>("IdUsuario")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Apelido")
                        .HasColumnType("longtext");

                    b.Property<string>("CPF")
                        .HasColumnType("longtext");

                    b.Property<int>("Celular")
                        .HasColumnType("int");

                    b.Property<DateTime>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCadastro")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("DataNascimento")
                        .HasColumnType("longtext");

                    b.Property<string>("Email")
                        .HasColumnType("longtext");

                    b.Property<string>("EnderecoUsuarioIdEndereco")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("IdEndereco")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("NomeCompleto")
                        .HasColumnType("longtext");

                    b.Property<string>("Senha")
                        .HasColumnType("longtext");

                    b.Property<int>("Sexo")
                        .HasColumnType("int");

                    b.HasKey("IdUsuario");

                    b.HasIndex("EnderecoUsuarioIdEndereco");

                    b.ToTable("Usuarios");
                });

            modelBuilder.Entity("BRQ_Ingresso.Models.Usuario", b =>
                {
                    b.HasOne("BRQ_Ingresso.Models.EnderecoUsuario", "EnderecoUsuario")
                        .WithMany()
                        .HasForeignKey("EnderecoUsuarioIdEndereco");

                    b.Navigation("EnderecoUsuario");
                });
#pragma warning restore 612, 618
        }
    }
}
